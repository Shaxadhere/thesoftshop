...
class Category{
    ...
    function ListRandom($Current, $limit){
        ...
        $Current = mysqli_real_escape_string(connect(), $Current);
        return mysqli_query(
            connect(),
            "SELECT * FROM tbl_category where Deleted = 0 and CategoryName <> '$Current' and CategoryName <> 'New Arrivals' and CategoryName <> 'Stationary' and CategoryName <> 'Accessories' and CategoryName <> 'Stickers' and CategoryName <> 'Featured' and CategoryName <> 'Happy Deals' ORDER BY RAND() LIMIT $limit"
        );
    }
}
